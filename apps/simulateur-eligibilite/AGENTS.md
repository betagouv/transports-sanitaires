...
`regles/regles.publicodes`) et `@publicodes/forms` (`FormBuilder` engendre le
formulaire depuis les règles).

Précédé d'un **écran-porte d'identification du prescripteur obligatoire**
(`front/identification/`, référentiel Grist). Servi par un **backend
...
prescription médicale de transport (n° 11574*07, réf. S3138g), `dap/` pour la
demande d'accord préalable (n° 11575*08, réf. S3139h). Ce qu'ils partagent —
l'écriture dans le PDF, la lecture du modèle, la forme d'un tableau — est d'un
cran au-dessus, dans `cerfa/`.
...
confronte chaque tableau à son gabarit : aucun champ ne peut être oublié ni
...
- **Les noms de règles passent par le contrat.**
  ...
  liste toutes les clés que le code a le droit de nommer ; `Cible` et
...
- **Questions à choix multiple** : elles s'encodent en `mosaique` — N règles
  booléennes plus une règle parente inerte portant la métadonnée, consommée par
...
- **Extensions d'import** : `.ts` partout où Node peut atteindre le fichier
  ...
  extensions ; Vite si. Ce n'est pas une affaire de dossier mais
  d'accessibilité, et `tests/lisibilite.test.ts` la calcule.
...
d'interface passent Testing Library sur le vrai `<App />` ; les tests serveur
...
*avec leurs cibles attendues*, en publicodes nu (donc lisible par Node aussi),
rejoué par la matrice de non-régression métier, parcourable dans la galerie de
seeds et utilisé par `npm run apercu-cerfa`.

## Les outils produit

La galerie de seeds et le **labo** de règles sont les deux *outils produit* :
même garde d'accès **sur tous les environnements** (service n° 4 du référentiel,
