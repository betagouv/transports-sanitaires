...
type VerdictContexte = { attenteRequise: boolean; estConvocation: boolean };
...
//
// Depuis la v9.7.3 (TS973-03), ce résultat couvre aussi l'avion/bateau hors
// convocation (« situations spéciales », famille AUD-AIR-ORIENTATION) : le
// corps se contextualise sur `estConvocation`, faute de quoi il prêterait au
// patient une convocation qu'il n'a pas (`CONTRAT-RESULTATS-v9-7-3.md` § 2).
export function orientationCaisse({
  attenteRequise,
  estConvocation,
}: VerdictContexte): Verdict {
  const paragraphes = [
    (estConvocation ? "Votre convocation" : "Votre déplacement") +
      " nécessite un transport en avion ou en bateau de ligne régulière. Les informations renseignées ne permettent pas de compléter les sous-situations prévues par le formulaire de demande d’accord préalable.",
    "Contactez votre caisse d’Assurance Maladie avec " +
      (estConvocation ? "votre convocation et " : "") +
      "cette synthèse afin de confirmer le document, les pièces à fournir et la personne qui doit établir la demande.",
...
export function EtapesOrientationCaisse({
  estConvocation,
}: Pick<VerdictContexte, "estConvocation">) {
  const etapes = [
    estConvocation
      ? "Conservez votre convocation et la synthèse remise ici."
      : "Conservez la synthèse remise ici.",
