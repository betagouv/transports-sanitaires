...
// au simulateur (voir docs/architecture/identification.md — ADR-1). Composant de
// pure sélection ; à la validation il remonte la `Selection` brute à `onValide`
// (c'est la porte, App.tsx, qui la convertit en contexte via l'API et bascule
// vers le simulateur). Le référentiel par défaut est le snapshot factice (dev /
// tests) ; en production App injecte le client HTTP same-origin.

import { useEffect, useState } from "react";
import { PRESCRIPTEUR_AUTRE, type Selection } from "../../shared/selection";
import {
  snapshotReferentiel,
  type Etablissement,
  type Prescripteur,
...
export function Identification({
  ...
  function handleValider() {
    onValide({ etabId, serviceId, prescripteurId });
  }
