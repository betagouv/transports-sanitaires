...
//
// Trois cas particuliers médicaux concluent dès la Partie 1 : le SMUR, la
// permission de sortie sans motif médical, et le seul motif bariatrique. Le
// modèle dit alors `cible_partie_2_requise = non`, et le questionnaire
// administratif n'a rien à poser.
//
...
import { estApplicable, evalue } from "./situations-v9-4-1";

// Les trois sorties directes, avec le cas final que chacune arrête. La base
// neutre répond par ailleurs à tout le questionnaire administratif : c'est le
// dossier qu'un secrétariat reprend, et le pire cas pour ces gardes.
const SORTIES_DIRECTES = [
  ...
  ["p1_m0_bariatrique", "bariatrique seul"],
] as const;
...
describe("PART2-GATE-001 — une sortie directe de la Partie 1 ferme la Partie 2", () => {
  it.each(SORTIES_DIRECTES)(
    ...
    (cas, casFinal) => {
      const moteur = evalue({ [cas]: "oui", p1_m0_aucun: "non" });
      ...
      expect(estApplicable(moteur, "p2_convocation_ou_avis")).toBe(false);
    },
  );
});
...
describe("PART2-GATE-002 — A2.1 attend une réponse explicite à A0.1", () => {
  it("reste fermée tant que le patient hospitalisé n'est pas tranché", () => {
    ...
    expect(estApplicable(moteur, "p2_convocation_ou_avis")).not.toBe(true);
  });

  it("s'ouvre dès qu'A0.1 est répondue « non »", () => {
    const moteur = evalue({ p2_patient_hospitalise: "non" });
    expect(estApplicable(moteur, "p2_convocation_ou_avis")).toBe(true);
  });
});
