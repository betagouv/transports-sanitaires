...
import { pseudonymise } from "../server/contexte.ts";
...
const decodeCtx = (encoded: string): Record<string, unknown> =>
...
describe("POST /api/contexte", () => {
  ...
  it("renvoie un contexte pseudonymisé (refs HMAC), sans identifiant brut ni PII", async () => {
    const { status, body } = await post("/api/contexte", selection);
    expect(status).toBe(200);

    const ctx = decodeCtx(body.ctx);
  ...
  it("est déterministe pour une même sélection", async () => {
    ...
    expect(a.body.ctx).toBe(b.body.ctx);
  });
